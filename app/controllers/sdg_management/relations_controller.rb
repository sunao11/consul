  ...
  private
    ...
    def update_notice
      if @record.sdg_review.present?
        flash[:notice] = t("sdg_management.relations.update.notice",
          relatable: relatable_class.model_name.human)
      else
        @record.create_sdg_review!
        flash[:notice] = t("sdg_management.relations.update_and_review.notice",
          relatable: relatable_class.model_name.human)
      end
    end
end
